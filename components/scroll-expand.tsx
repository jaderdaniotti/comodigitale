...
export interface ScrollExpandProps {
  ...
  className?: string;
  style?: CSSProperties;
  /** Constant tint over the media (e.g. bg-ink/20). */
  mediaTintClassName?: string;
...
const ScrollExpand = ({
  ...
  const media =
    mediaType === "video" ? (
      ...
      // eslint-disable-next-line @next/next/no-img-element -- scroll transform needs direct DOM ref
      <img
        ...
        >
          ...
          >
            ...
            {mediaTintClassName ? (
              <div
                aria-hidden
                className={`pointer-events-none absolute inset-0 ${mediaTintClassName}`}
              />
            ) : null}
            <div
              ref={scrimRef}
          ...
          {scrollHint ? (
            <div
              ref={hintRef}
              className={`pointer-events-none absolute inset-x-0 bottom-5 text-center text-[0.8125rem] tracking-[0.02em] [will-change:opacity,transform] ${
